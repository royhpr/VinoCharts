...
-(id)initWithModel:(Project*)p Delegate:(id)d {
    ...
    if (self != nil) {
        ...
        // We want to prevent long details from overflowing the view.
        NSInteger numLineBreaks = [[details componentsSeparatedByCharactersInSet:
                             [NSCharacterSet newlineCharacterSet]] count];
        if (numLineBreaks > MAX_LINE_BREAKS) {
            int currentNumOfLineBreaks = 0;
            int maxChar = 0;
            for (int i=0; i<[details length]; i++) {
                if ([[details substringWithRange:NSMakeRange(i, 1)] isEqualToString:@"\n"]) {
                    currentNumOfLineBreaks += 1;
                }
                if (currentNumOfLineBreaks == MAX_LINE_BREAKS) {
                    maxChar = i;
                    break;
                }
            }
            details = [[details substringToIndex:maxChar] stringByAppendingString:@" ..."];
        } else if ([details length] > DESC_LENGTH) {
