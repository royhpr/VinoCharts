...
+(BOOL)isView:(UIView*)subV completelyInside:(UIView*)superV{
    ...
    double subVTLX = subV.frame.origin.x;
    double subVTLY = subV.frame.origin.y;
    double subVWidth = subV.frame.size.width;
    double subVHeight = subV.frame.size.height;
    double subVBRX = subVTLX + subVWidth;
    double subVBRY = subVTLY + subVHeight;
    
    if (subVTLX >=0
        && subVTLY >=0
        && subVBRX <= superV.frame.size.width
        && subVBRY <= superV.frame.size.height)
        return YES;
    else
        return NO;
}
...
+(BOOL)isPoint:(CGPoint)pt TouchingRect:(CGRect)frame1{
    if (frame1.origin.x <= pt.x
        && pt.x <= frame1.origin.x+frame1.size.width
        && frame1.origin.y <= pt.y
        && pt.y <= frame1.origin.y+frame1.size.height) {
        return YES;
    }
    else
        return NO;
