...
-(void)syncWithDropBox {
    if (![[DBSession sharedSession] isLinked]) {
        ...
        self.timer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(transitIfSuccesfullySignIn) userInfo:nil repeats:YES];
        
        //in the case where user cancel logging in to dropbox, the timer continue running
