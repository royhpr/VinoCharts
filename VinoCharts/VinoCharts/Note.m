...
-(id)initWithText:(NSString*)t{
    ...
    if(self = [super init]){
        ...
        _view = [[NoteView alloc]initWithFrame:CGRectMake(-(NOTE_DEFAULT_WIDTH/2.0),
                                                          -(NOTE_DEFAULT_HEIGHT/2.0), NOTE_DEFAULT_WIDTH ,NOTE_DEFAULT_HEIGHT) AndText:t]; //statement [A]
        
        _view.bounds = CGRectMake(0, 0, NOTE_DEFAULT_WIDTH, NOTE_DEFAULT_HEIGHT);
        
        [_view setFont:[UIFont fontWithName:fHELVETICA_NEUE size:NOTE_DEFAULT_FONT_SIZE]];
		...
		_body.pos = cpv(0, 0); //Ties up with statement [A] (see above).
...
//temporary TODO fix Civics wasn't able to implement saving UIColor objects.
...
-(void)addBold{
    UIFont* font = [self getFont];
    if ([FontHelper isNotImbuedWithModifier:font] || [FontHelper isOnlyItalics:font]){
        font = [FontHelper addBoldTo:font];
        [self setFont:font];
    }
}
...
-(void)addItalics{
    UIFont* font = [self getFont];
    if ([FontHelper isNotImbuedWithModifier:font] || [FontHelper isBold:font]) {
        font = [FontHelper addItalicsTo:font];
        [self setFont:font];
    }
}
